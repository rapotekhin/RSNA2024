...
def train_net(net, train_dataloader, train, train_dataset, val_dataloader, val, val_dataset, device):
    ...
    #         target = batch['target'].repeat(in_tensor.shape[0], 1, 1)  # Repeat target to match input batch siz
            ...
            if (batch_idx + 1) % (len(train_dataloader) // 5) == 0:
                ...
                message = f"Epoch {epoch+1}, Train  Loss: {print_loss:.4f}"
        ...
        if val_metric < best_val_metric:
            ...
            torch.save({"state_dict": net.state_dict(), "val_metric": val_metric}, "model_best.pth")
        ...
        # Logging
        message = f"Epoch {epoch+1}, Loss: {epoch_loss:.4f}, Val Metric: {val_metric:.4f}, LR: {optimizer.param_groups[0]['lr']:.6f}"
    ...
    net.load_state_dict(torch.load("model_best.pth", map_location="cpu")["state_dict"])
    net = net.to(device)
